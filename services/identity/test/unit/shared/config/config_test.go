...
// setConfigTestEnvironment provides the baseline configuration required by
// config.Load() during tests.
//
// These are test-only values and must never be used as production secrets.
func setConfigTestEnvironment(t *testing.T) {
	t.Helper()
...
func TestLoad_ReturnsConfigurationFromEnvironment(t *testing.T) {
	...
	assert.Equal(t, "gateway", cfg.ServiceAuthName)
	assert.Equal(t, "test-service-secret", cfg.ServiceAuthSecret)
}
